...
document.getElementById('form-identificacao').addEventListener('submit', async (e) => {
  ...
  const jaFez = JSON.parse(localStorage.getItem('hedra_v2_participantes') || '[]');
  ...
  try {
    ...
    if (jaFez.includes(email)) {
      ...
      localStorage.setItem('hedra_v2_participantes', JSON.stringify(atualizado));
    }
  } catch (_) {
    if (jaFez.includes(email)) {
  ...
  try {
    ...
    if (json.status === 'ok') {
      ...
      if (!jaFez.includes(userData.email)) {
        ...
        localStorage.setItem('hedra_v2_participantes', JSON.stringify(jaFez));
      }
    }
  } catch (err) {
